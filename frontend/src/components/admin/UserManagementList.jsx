import { mfaLabel } from './userStatus.js'
...
export default function UserManagementList({ users, currentUserId, loading, selectedId, onSelect, emptyMessage }) {
  ...
  return <div className="admin-users-list"><div className="admin-table-head" aria-hidden="true"><span>Kullanıcı</span><span>Rol</span><span>Durum</span><span>E-posta</span><span>2FA</span></div>{users.map((u) => <button type="button" className={`admin-user-row ${selectedId === u.id ? 'is-selected' : ''}`} key={u.id} onClick={() => onSelect(u.id)} aria-label={`${u.username} kullanıcısının detayını aç`}>
    ...
    <span data-label="Rol"><Badge tone={u.role === 'Admin' ? 'purple' : ''}>{u.role || 'Atanmamış'}</Badge></span><span data-label="Durum"><Badge tone={u.isActive ? 'success' : 'danger'}>{u.isActive ? 'Aktif' : 'Pasif'}</Badge></span><span data-label="E-posta"><Badge tone={u.emailConfirmed ? 'success' : 'warning'}>{u.emailConfirmed ? 'Doğrulandı' : 'Doğrulanmadı'}</Badge></span><span data-label="2FA"><Badge tone={u.twoFactorEnabled ? 'success' : 'warning'}>{mfaLabel(u)}</Badge></span>
  </button>)}</div>
}
