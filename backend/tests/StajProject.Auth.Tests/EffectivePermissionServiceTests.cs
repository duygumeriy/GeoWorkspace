...
public class EffectivePermissionServiceTests
{
    ...
    public async Task Administrator_permissions_come_from_rows_not_from_the_role_name()
    {
        ...
        var service = Service(scope);
        Assert.Equal(27, (await service.GetEffectivePermissionCodesAsync(user.Id)).Count);
        ...
        Assert.Equal(26, (await service.GetEffectivePermissionCodesAsync(user.Id)).Count);
    }
